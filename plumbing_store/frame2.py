...
def save_invoice_to_file():
    ...
    if op:
        ...
        filename = "فاتورة.xlsx"

        # إنشاء DataFrame
        levels = [
            ["اسم المحل", "الصفا"],
            ["جمال ابراهيم (المحل)", "01025812907"],
            ["احمد الإمام (السياره)", "01090179105"],
            ['اسم المشترى', name],
            ['رقم المشترى', phone],
            ['التاريخ', timestamp],
            ['القيمة', ' السعر بعد الخصم', 'سعر مبدئي', 'الكمية', 'اسم الصنف']
        ]
        columns = pd.MultiIndex.from_product(levels, names=["الصفا", "01025812907", "01090179105", name, phone, timestamp, 'detail'])
        ...
        if numeric_columns.empty:
            total_values = [None] * len(df.columns)
        else:
            total_values = [totals.get(col, None) for col in numeric_columns]
            total_values.append('الإجمالي')
        ...
        # حفظ DataFrame إلى Excel
        df.to_excel(filename, index=True, engine='openpyxl')

        # فتح الملف باستخدام openpyxl وتعديل العرض للأعمدة
        workbook = load_workbook(filename)
        sheet = workbook.active
        
        # تعيين عرض مخصص لكل عمود
        for col in sheet.columns:
            max_length = 0
            column_letter = None

            # تحقق من نوع الخلية قبل محاولة الوصول إلى column_letter
            for cell in col:
                if isinstance(cell, MergedCell):
                    continue  # تخطي الخلايا المدمجة
                if column_letter is None:
                    column_letter = cell.column_letter  # الحصول على الحرف الخاص بالعمود
            ...
            if column_letter:
                adjusted_width = (max_length + 2)  # إضافة مسافة إضافية لتوسيع العمود
                sheet.column_dimensions[column_letter].width = adjusted_width

        # إضافة حدود للصفوف لإبراز الفاصل بين القيم
        ...
        # إضافة ألوان خلفية
        total_fill = PatternFill(start_color="FFFF00", end_color="FFFF00", fill_type="solid")  # لون أصفر
        title_fill = PatternFill(start_color="00FF00", end_color="00FF00", fill_type="solid")   # لون أخضر

        # تطبيق الحدود والألوان على الصفوف
        for row in sheet.iter_rows(min_row=2, max_row=sheet.max_row, min_col=1, max_col=sheet.max_column):
            for cell in row:
                cell.border = thin_border

                if cell.row == sheet.max_row:  # الصف الذي يحتوي على العنوان
                    cell.fill = title_fill
                
        # حذف الصف 8 إذا لزم الأمر
        sheet.delete_rows(8)
        ...
        sheet.cell(row=last_row, column=5, value="العنوان: بني عبيد أمام مسجد الشوادي البر الأيسر . الدقهليه")
        for column in range(1, sheet.max_column + 1):  # بدءًا من العمود 1 إلى العمود الأخير
